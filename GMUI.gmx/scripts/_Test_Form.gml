...
// Left side group
GMUI_CreateGroup(2,      2,3,    GMUIAnchor.TopLeft);
...
with (GMUI_Add("Test2", "textint",              0,0,    10,2,   GMUIAnchor.TopLeft)) {
...
with (GMUI_Add("Test3", "textdecimal",          0,3,    10,2,   GMUIAnchor.TopLeft)) {
    GMUI_ControlSetAttributes(10,10,-999,999);
    GMUI_ControlAddToGroup(2);
    //GMUI_ControlSetSpriteMap(s2,s3,s4,s5,s6,s7,s8,s1,s9,false);
}

with (GMUI_Add("Test4", "intpicker",            0,6,    10,2,   GMUIAnchor.TopLeft)) {
...
with (GMUI_Add("TestButton", "textbutton",      3,9,    5,3,    GMUIAnchor.TopLeft)) {
...
with (GMUI_Add("DebugButton", "textbutton",     1,3,    5,3,    GMUIAnchor.BottomLeft)) {
...
with (GMUI_Add("MoveButton", "textbutton",      22,3,   6,3,    GMUIAnchor.BottomLeft)) {
    GMUI_ControlSetButtonAction(_Move_Button);
    GMUI_ControlSetButton("Move"+chr(13)+"Group",-1,-1,-1);
}

with (GMUI_Add("EnlargeButton", "textbutton",   30,3,   6,3,    GMUIAnchor.BottomLeft)) {
    GMUI_ControlSetButtonAction(_Enlarge_Button);
    GMUI_ControlSetButton("Enlarge"+chr(13)+"Window",-1,-1,-1);
}

with (GMUI_Add("MoveButton2", "button",         18,0,   2,2,    GMUIAnchor.TopLeft)) {
    GMUI_ControlSetButtonAction(_Move_Button);
    GMUI_ControlSetButton("", GMUIspr_move, -1, -1);
    GMUI_ControlAddToGroup(1);
}

with (GMUI_Add("SwitchButton", "button",        -2,16,  4,2,    GMUIAnchor.Top)) {
...
// Test slider
with (GMUI_Add("Slider", "slider",              16,12,  10,2,   GMUIAnchor.BottomRight)) {
...
with (GMUI_Add("MidSlider", "button",           19,12,  2,2,    GMUIAnchor.BottomRight)) {
...
with (GMUI_Add("SnapSlider", "button",          23,12,  3,2,    GMUIAnchor.BottomRight)) {
...
with (GMUI_Add("CheckBox", "checkbox",          4,12,   1,1,    GMUIAnchor.BottomLeft)) {
...
with (GMUI_Add("Toggle", "toggle",              3,10,   3,2,    GMUIAnchor.BottomLeft)) {
...
with (GMUI_Add("SpritePicker", "spritepicker",  -10,10, 5,2,    GMUIAnchor.Bottom)) {
    ...
    GMUI_ControlSetPicker(sprite_get_width(GMUIspr_arrowleft) + 4,sprite_get_height(GMUIspr_arrowleft) + 4,
        GMUIDirection.Horizontal, GMUIspr_arrowright, GMUIspr_arrowleft);
    GMUI_ControlSetInitValue(6);
}
// Example: overrides initial value:
GMUI_ControlSelectOption("SpritePicker",2);

//with (GMUI_Add("Graphic", "image",          -12,10, 2,2,    GMUIAnchor.Bottom)) {
...
GMUI_CreateGroup(3,          0,0,    GMUIAnchor.TopLeft);


with (GMUI_Add("MenuInt", "intpicker",          0,0,    3,2,    GMUIAnchor.TopLeft)) {
